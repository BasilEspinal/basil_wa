...
const openForm = (mode, rowData) => {
    ...
    formProperties.value = {
        open: true,
        title: mode === 'Ver Detalles' || mode === 'details' ? 'Detalles de Tipo de Ejecución' : 'Formulario',
        mode: mode,
...
const Compan = ref([]);
const compa = ref([]);
const work_type_tarif_list = ref([
...
const openDialog = (mode, rowData) => {
    ...
    if (mode === 'new') {
        ...
        resetForm();
        const { code, company: empresa, farm: finca, name: nombre, work_type_tarif: tarif, calculate_work_type: calculatewt } = source;

        name.value = nombre;
        codeV.value = code;
        if (tarif) work_type_tarifV.value = { id: tarif, name: tarif };
        if (calculatewt) calculate_work_typeV.value = { id: calculatewt, name: calculatewt };
        if (empresa) company.value = { id: empresa.uuid, name: empresa.name };
        if (finca) farm.value = { id: finca.uuid, name: finca.name };
    }
        ...
        <div class="card mb-4 bg-primary-reverse border-round-xl shadow-2">
            <div class="flex align-items-center justify-content-between p-3">
                <div class="flex align-items-center gap-3">
                    ...
                    <div>
                        <h1 class="m-0 text-3xl font-bold tracking-tight">{{ $t('menu.workTypes') }}</h1>
                        <p class="m-0 text-600 font-medium mt-1">Configuración de tipos de ejecución de labores</p>
                    </div>
                </div>
            </div>
        </div>
