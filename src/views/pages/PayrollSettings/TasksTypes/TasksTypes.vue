...
/**
 * Opens the form dialog for creating or viewing task type details.
 * @param {string} mode - The mode in which to open the form ('new', 'details', etc.).
 * @param {Object} [rowData] - Optional task type record.
 */
const openForm = (mode, rowData) => {
    ...
    formProperties.value = {
        open: true,
        title: mode === 'Ver Detalles' || mode === 'details' ? 'Detalles de Tipo de Tarea' : 'Formulario',
        mode: mode,
...
/**
 * Fetches task type records from the API using CrudService.
 */
...
/**
 * Handles batch patching of status for selected task types.
 */
        ...
        <div class="card mb-4 bg-primary-reverse border-round-xl shadow-2">
            <div class="flex align-items-center justify-content-between p-3">
                <div class="flex align-items-center gap-3">
                    ...
                    <div>
                        <h1 class="m-0 text-3xl font-bold tracking-tight">{{ $t('menu.taskTypes') }}</h1>
                        <p class="m-0 text-600 font-medium mt-1">Definición de tipos y categorías de labores</p>
                    </div>
                </div>
            </div>
        </div>
