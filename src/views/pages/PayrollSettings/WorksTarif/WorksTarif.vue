...
const dynamicColumns = [
    ...
    { field: 'tasks_of_type.name', header: 'Task Name', frozen: true, color: false },
...
const openForm = (mode, rowData) => {
    ...
    formProperties.value = {
        open: true,
        title: mode === 'Ver Detalles' || mode === 'details' ? 'Detalles de Tarifa' : 'Formulario',
        mode: mode,
...
const openDialog = (mode, rowData) => {
    ...
    if (mode === 'new') {
        ...
        resetForm();
        const { company: empresa, farm: farmParameter, tasks_of_type: task, done_type: done, work_type_day: work, work_type_tarif: tarif, price_tarif: price } = source;

        if (task) tasks_of_typeV.value = { id: task.uuid, name: task.name };
        if (done) done_typeV.value = { id: done.uuid, name: done.name };
        if (work) work_type_dayV.value = { id: work, name: work };
        if (tarif) work_type_tarifV.value = { id: tarif, name: tarif };
        ...
        <div class="card mb-4 bg-primary-reverse border-round-xl shadow-2">
            <div class="flex align-items-center justify-content-between p-3">
                <div class="flex align-items-center gap-3">
                    ...
                    <div>
                        <h1 class="m-0 text-3xl font-bold tracking-tight">{{ $t('menu.laborRates') }}</h1>
                        <p class="m-0 text-600 font-medium mt-1">Definición de tarifas y costos por labor</p>
                    </div>
                </div>
            </div>
        </div>
