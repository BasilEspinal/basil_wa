...
const openDialog = (mode) => {
    ...
    if (mode === 'new') {
        ...
        resetForm();
        const { code, company: empresa, farm: farmParameter, name: nombre, taskoftype } = listRowSelect.value[0];

        name.value = nombre;
        codeV.value = code;
        taskTypeV.value = { id: taskoftype.uuid, name: taskoftype.name };
