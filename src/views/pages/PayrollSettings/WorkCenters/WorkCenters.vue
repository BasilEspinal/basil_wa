    ...
    <div class="card">
        <div class="grid">
            ...
            <Toolbar style="margin-bottom: 1rem">
                <template #center>
                    <Button v-if="ability.can('centro_trabajo_crear')" :disabled="headerNames.length > 0" label="New" icon="pi pi-plus" class="p-button-success mb-2 mt-2" @click="openNew" size="large" />
                    <Divider v-if="ability.can('centro_trabajo_crear')" layout="vertical" />
                    <Button v-if="ability.can('centro_trabajo_editar')" :disabled="!(listRowSelect.length > 0 && listRowSelect.length < 2)" label="Edit" icon="pi pi-file-edit" class="p-button-help mb-2 mt-2" @click="openEdit" size="large" />
                    <Divider v-if="ability.can('centro_trabajo_editar')" layout="vertical" />
                    <Button v-if="ability.can('centro_trabajo_crear')" :disabled="!(listRowSelect.length > 0 && listRowSelect.length < 2)" label="Clone" icon="pi pi-copy" class="p-button-secondary mb-2 mt-2" @click="openClone" size="large" />
                    <Divider v-if="ability.can('centro_trabajo_crear')" layout="vertical" />
                    <Button v-if="ability.can('centro_trabajo_crear')" :disabled="headerNames.length > 0" label="Export" icon="pi pi-file-import" class="p-button-warning mb-2 mt-2" @click="openExport" size="large" />
                    <Divider v-if="ability.can('centro_trabajo_crear')" layout="vertical" />
                    <Button v-if="ability.can('centro_trabajo_eliminar')" :disabled="!listRowSelect.length > 0" label="Delete" icon="pi pi-trash" class="p-button-danger mb-2 mt-2" @click="openDelete" size="large" />
                </template>
            </Toolbar>
            </div>
