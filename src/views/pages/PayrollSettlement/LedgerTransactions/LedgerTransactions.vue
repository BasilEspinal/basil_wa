...
const dynamicColumns = [
    ...
    { field: 'transaction_type', header: 'Transaction Type', frozen: false, color: false },
    { field: 'task_total_pays', header: 'Total Pays', frozen: false, color: false },
    ...
    { field: 'planner_tasks.transaction_date', header: 'Planner Date', frozen: false, color: false },
...
const openForm = (mode, rowData) => {
    ...
    formProperties.value = {
        open: true,
        title: mode === 'Ver Detalles' || mode === 'details' ? 'Detalles de Transacción Ledger' : 'Formulario',
        mode: mode,
...
let endpoint = ref(`/transactions/ledger/ledgers`); //replace endpoint with your endpoint
    ...
    if (row) {
        cardSections.value = [
            ...
            },
            {
                ...
                fields: {
                    'Planner Date': row.planner_tasks?.transaction_date
                },
...
const { itemsActions } = useActions(`/transactions/ledger/ledgers`);

const openDialog = (mode, rowData) => {
    formDialogTitle.value = mode === 'new' ? 'Nueva Transacción Ledger' : mode === 'edit' ? 'Editar Transacción' : mode === 'clone' ? 'Clonar Transacción' : mode === 'patch' ? 'Actualizar Transacción' : '';
            ...
            <div class="flex align-items-center justify-content-between p-3">
                <div class="flex align-items-center gap-3">
                    <div class="bg-primary-50 p-3 border-round-circle">
                        <i class="pi pi-wallet text-primary text-3xl"></i>
                    </div>
                    <div>
                        <h1 class="m-0 text-3xl font-bold tracking-tight">{{ $t('menu.payrollSettlement.LedgerTransactions.title') }}</h1>
                        <p class="m-0 text-600 font-medium mt-1">Gestión de transacciones contables</p>
                    </div>
                </div>
            </div>
        </div>
