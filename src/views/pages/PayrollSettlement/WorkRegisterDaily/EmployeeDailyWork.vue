...
const namePage = ' Labores de contratista ';
    ...
    <div>
        <div class="card mb-4 bg-primary-reverse border-round-xl shadow-2">
            <div class="flex align-items-center justify-content-between p-3">
                <div class="flex align-items-center gap-3">
                    ...
                    <div>
                        <h1 class="m-0 text-3xl font-bold tracking-tight">Labores de Contratista</h1>
                        <p class="m-0 text-600 font-medium mt-1">Gestión y seguimiento de labores realizadas por contratistas</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card">
                ...
                <div class="col-xs-12 col-sm-6 col-md-4 mb-2 text-center mx-auto">
                    ...
                    <div class="col-xs-12 col-sm-6 col-md-4 mb-2 text-center mx-auto">
                        <Toolbar style="margin-bottom: 1rem">
                            <template #center>
                                <Button v-if="ability.can('trabajo_contratista_crear')" :disabled="headerNames.length > 0" label="New" icon="pi pi-plus" class="p-button-success mb-2 mt-2" @click="openNew" size="large" />
                                <Divider v-if="ability.can('trabajo_contratista_crear')" layout="vertical" />
                                <Button
                                    v-if="ability.can('trabajo_contratista_editar')"
                                    ...
                                    size="large"
                                />
                                <Divider v-if="ability.can('trabajo_contratista_editar')" layout="vertical" />
                                <Button
                                    v-if="ability.can('trabajo_contratista_crear')"
                                    :disabled="!(listRowSelect.length > 0 && listRowSelect.length < 2)"
                                    label="Clone"
                                    icon="pi pi-copy"
                                ...
                                />
                                <Divider v-if="ability.can('trabajo_contratista_crear')" layout="vertical" />
                                <Button v-if="ability.can('trabajo_contratista_crear')" :disabled="headerNames.length > 0" label="Export" icon="pi pi-file-import" class="p-button-warning mb-2 mt-2" @click="openExport" size="large" />
                                <Divider v-if="ability.can('trabajo_contratista_crear')" layout="vertical" />
                                <Button v-if="ability.can('trabajo_contratista_eliminar')" :disabled="!listRowSelect.length > 0" label="Delete" icon="pi pi-trash" class="p-button-danger mb-2 mt-2" @click="openDelete" size="large" />
                            </template>
                        </Toolbar>
                    </div>
                </div>
            ...
            <DataTable
                ...
                v-if="ability.can('trabajo_contratista_listado')"
            >
                <template #header>
                    <div class="flex flex-column md:flex-row md:justify-content-between md:align-items-center gap-3">
                        <div class="flex gap-2 align-items-center">
                            ...
                            <span class="p-input-icon-left">
                                ...
                                <InputText v-model="filters['global'].value" placeholder="Buscar labor..." class="w-full md:w-20rem" />
                            </span>
                        </div>
                        ...
                        <div class="flex gap-2">
                            <Button v-if="ability.can('trabajo_contratista_crear')" icon="pi pi-plus" class="p-button-raised p-button-success p-button-rounded" @click="openNew" v-tooltip.top="'Nuevo Registro'" />
                            <Button v-if="ability.can('trabajo_contratista_crear')" icon="pi pi-file-export" class="p-button-outlined p-button-secondary p-button-rounded" @click="openExport" :disabled="!dataFromComponent || dataFromComponent.length === 0" v-tooltip.top="'Exportar'" />
                        </div>
                    </div>
                </template>
                ...
                <Column header="Acciones" :frozen="true" alignFrozen="right" style="min-width: 12rem">
                    <template #body="{ data }">
                        <div class="flex gap-2">
                            <Button v-if="ability.can('trabajo_contratista_editar')" icon="pi pi-pencil" class="p-button-rounded p-button-text p-button-warning" @click="openEdit" v-tooltip.top="'Editar'" />
                            <Button v-if="ability.can('trabajo_contratista_crear')" icon="pi pi-copy" class="p-button-rounded p-button-text p-button-secondary" @click="openClone" v-tooltip.top="'Clonar'" />
                            <Button v-if="ability.can('trabajo_contratista_eliminar')" icon="pi pi-trash" class="p-button-rounded p-button-text p-button-danger" @click="openDelete" v-tooltip.top="'Eliminar'" />
                        </div>
                    </template>
                </Column>
            </DataTable>
