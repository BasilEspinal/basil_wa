...
const openForm = (mode, rowData) => {
    ...
    formProperties.value = {
        open: true,
        title: mode === 'Ver Detalles' || mode === 'details' ? 'Detalles de Transacción (Contratista)' : 'Formulario',
        mode: mode,
...
const openDialog = (mode, rowData) => {
    formDialogTitle.value = mode === 'new' ? 'Nueva Transacción de Contratista' : mode === 'edit' ? 'Editar Transacción' : mode === 'clone' ? 'Clonar Transacción' : mode === 'patch' ? 'Actualizar Transacción' : '';
        ...
        <div class="card mb-4 bg-primary-reverse border-round-xl shadow-2">
            <div class="flex align-items-center justify-content-between p-3">
                <div class="flex align-items-center gap-3">
                    ...
                    <div>
                        <h1 class="m-0 text-3xl font-bold tracking-tight">{{ $t('menu.dailyTransactionsContractor') }}</h1>
                        <p class="m-0 text-600 font-medium mt-1">Gestión de transacciones para contratistas</p>
                    </div>
                </div>
            </div>
        </div>
