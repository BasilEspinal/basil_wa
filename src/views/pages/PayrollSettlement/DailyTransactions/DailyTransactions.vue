...
const openForm = (mode, rowData) => {
    ...
    formProperties.value = {
        open: true,
        title: mode === 'Ver Detalles' || mode === 'details' ? 'Detalles de Transacción Diaria' : 'Formulario',
        mode: mode,
        data: mode === 'detalles' || mode === 'details' ? null : (rowData || listRowSelect.value[0])
    };
};

let endpoint = ref('/transactions/tasks?page=' + '1'); //replace endpoint with your endpoint
        ...
        cardSections.value = [
            {
                ...
                fields: {
                    ...
                    'Price Tarif Task': row.price_tarif_task,
            ...
            },
            {
                ...
                fields: {
                    'Vehicle Code': row.vehicle?.code,
                    'Vehicle Type': row.vehicle?.vehicle_type
                },
                icon: 'pi pi-car',
            ...
            },
            {
                ...
                fields: {
                    'Customer Name': row.customer_request_id?.customer_name
                },
                icon: 'pi pi-users',
                bgColor: 'bg-lime-100',
        ...
        <div class="card mb-4 bg-primary-reverse border-round-xl shadow-2">
            <div class="flex align-items-center justify-content-between p-3">
                <div class="flex align-items-center gap-3">
                    ...
                    <div>
                        <h1 class="m-0 text-3xl font-bold tracking-tight">{{ $t('menu.dailyTransactions') }}</h1>
                        <p class="m-0 text-600 font-medium mt-1">Gestión de transacciones y novedades diarias</p>
                    </div>
                </div>
            </div>
        </div>
