...
const readAll = async () => {
    ...
    await getAllResponseAPI('/companies');
    Compan.value = dataResponseAPI.value.data.map((comp) => ({ id: comp.uuid, name: comp.name }));
};
...
const EditVarieties = handleSubmitNew(async (values) => {
    ...
    console.log(data);
    formDialogNew.value = false;
    await putResponseAPI(data, endpoint.value, uuid);
    const restp = dataResponseAPI.value.data;
    console.log('data: ', restp);
    toast.add({ severity: restp ? 'success' : 'error', summary: 'Create User ' + values.name, detail: restp ? 'Creado' : 'Error', life: 3000 });
    loadingData();
...
const DeleteVarieties = async () => {
    ...
    try {
        ...
        listRowSelect.value.forEach((item) => {
            const data = ({});
            const deletePromise = deleteResponseAPI(data, endpoint.value, item.uuid);
