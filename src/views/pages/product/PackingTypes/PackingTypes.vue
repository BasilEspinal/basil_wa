...
const dynamicColumns = [
    ...
    { field: 'code_dispatch', header: 'Dispatch Code', frozen: false, color: false },
    { field: 'weight_tare', header: 'Weight Tare', frozen: false, color: false },
...
const openForm = (mode, rowData) => {
    ...
    formProperties.value = {
        open: true,
        title: mode === 'Ver Detalles' || mode === 'details' ? 'Detalles de Tipo de Empaque' : 'Formulario',
        mode: mode,
        data: mode === 'detalles' || mode === 'details' ? null : (rowData || listRowSelect.value[0])
    };
};

let endpoint = ref('/packing_types'); //replace endpoint with your endpoint
...
const formDialogExportTitle = 'Export record';
const formDialogDeleteTitle = 'Delete record';
        ...
        cardSections.value = [
            {
                ...
                fields: {
                    Name: row.name,
                    Code: row.code,
                    'Dispatch Code': row.code_dispatch,
                    'Weight Tare': row.weight_tare,
...
const openDialog = (mode, rowData) => {
    ...
    if (mode === 'new') {
        ...
        resetForm();
        const { company: empresa, code, farm: finca, name: nombre, weight_tare: weight_tare, code_dispatch: code_dispatch } = source;
            ...
            <div class="flex align-items-center justify-content-between p-3">
                <div class="flex align-items-center gap-3">
                    <div class="bg-primary-50 p-3 border-round-circle">
                        <i class="pi pi-inbox text-primary text-3xl"></i>
                    </div>
                    <div>
                        <h1 class="m-0 text-3xl font-bold tracking-tight">{{ $t('menu.packingTypes') }}</h1>
                        <p class="m-0 text-600 font-medium mt-1">Gestión de tipos de empaque</p>
                    </div>
                </div>
            </div>
        </div>
        <div class="card">
            ...
            <Dialog v-model:visible="formDialog" modal :header="formDialogTitle" class="p-fluid text-center mx-auto">
                ...
                <div class="mb-3">
                    ...
                    <FrontEndErrors :errorsNew="errorsNew" name="weight_tareV" />
                    <BackendErrors :name="errorResponseAPI?.errors?.weight_tare" />
                </div>
                ...
                <div class="mb-3">
                    ...
                    <FrontEndErrors :errorsNew="errorsNew" name="code_dispatchV" />
                    <BackendErrors :name="errorResponseAPI?.errors?.code_dispatch" />
                </div>
                ...
                <div class="mb-3">
                    ...
                    <FrontEndErrors :errorsNew="errorsNew" name="company_uuid" />
                </div>
