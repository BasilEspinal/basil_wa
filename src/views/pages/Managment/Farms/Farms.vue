    ...
    <div class="card">
        <div class="grid">
            ...
            <Toolbar style="margin-bottom: 1rem">
                <template #center>
                    <Button v-if = "ability.can('finca_crear')" :disabled="headerNames.length > 0" label="New" icon="pi pi-plus" class="p-button-success mb-2 mt-2" @click="openNew" size="large" />
                    <Divider v-if = "ability.can('finca_crear')" layout="vertical" />
                    <Button v-if = "ability.can('finca_editar')" :disabled="!(listRowSelect.length > 0 && listRowSelect.length < 2)" label="Edit" icon="pi pi-file-edit" class="p-button-help mb-2 mt-2" @click="openEdit" size="large" />
                    <Divider v-if = "ability.can('finca_editar')" layout="vertical" />
                    <Button v-if = "ability.can('finca_crear')" :disabled="!(listRowSelect.length > 0 && listRowSelect.length < 2)" label="Clone" icon="pi pi-copy" class="p-button-secondary mb-2 mt-2" @click="openClone" size="large" />
                    <Divider v-if = "ability.can('finca_crear')" layout="vertical" />
                    <Button v-if = "ability.can('finca_crear')" :disabled="headerNames.length > 0" label="Export" icon="pi pi-file-import" class="p-button-warning mb-2 mt-2" @click="openExport" size="large" />
                    <Divider v-if = "ability.can('finca_crear')" layout="vertical" />
                    <Button v-if = "ability.can('finca_eliminar')" :disabled="!listRowSelect.length > 0" label="Delete" icon="pi pi-trash" class="p-button-danger mb-2 mt-2" @click="openDelete" size="large" />
                </template>
            </Toolbar>
            </div>
        ...
        :globalFilterFields="['name', 'company.name', 'status.name', 'created_at', 'updated_at']" 
        v-if = "ability.can('finca_listado')"
        >
        <template #header>
...
let endpoint = ref('/farms');  //replace endpoint with your endpoint
...
const openEdit = () => {
    resetForm();
    const { code, company: empresa, name: nombre } = listRowSelect.value[0];
...
const openClone = () => {
    resetForm();
    const { company: empresa, name: nombre } = listRowSelect.value[0];
