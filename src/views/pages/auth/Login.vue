...
import { useAuthService } from '@/service/auth/auth_service';
...
const { errors, defineField } = useForm({
  initialValues: { email: import.meta.env.VITE_APP_DEF_EMAIL || '', password: import.meta.env.VITE_APP_DEF_PASSWORD || '' },
...
const onSubmit = async () => {
  ...
  if (!result?.ok) {
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: String(result?.error || 'Credenciales incorrectas'),
      life: 3000,
    });
    return;
  }
        ...
        <div class="w-full py-7 px-3 sm:px-6" style="border-radius: 153px; background-color: var(--paleta-100)">
          ...
          <div>
            <form @submit.prevent="onSubmit">
              ...
              <label class="block text-l mb-2" :class="{ 'text-red-700': errors.email }">
                {{ errors.email }}
              </label>
              ...
              <label class="block text-l mb-2" :class="{ 'text-red-700': errors.password }">
                {{ errors.password }}
              </label>
...
<style scoped>
.pi-eye { transform: scale(1.6); margin-right: 1rem; }
.pi-eye-slash { transform: scale(1.6); margin-right: 1rem; }
</style>
