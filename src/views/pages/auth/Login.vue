...
const fetchInfoPostLogin = async (data) => {
    try {
        ...
        const { Employee, company_id, farm_id } = response;
        const { workCenter } = Employee;

        sessionStorage.setItem('accessSessionWorkCenter', JSON.stringify(workCenter));
        sessionStorage.setItem('accessSessionCompanyId', company_id);
        sessionStorage.setItem('accessSessionFarmId', farm_id);
